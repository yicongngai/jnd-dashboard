...
(function(){
  ...
  (function(){ var box=document.getElementById('band'); var rows=[].slice.call(document.querySelectorAll('tr.nb')).map(function(tr){ return {name:tr.querySelector('b').textContent, a:+tr.dataset.psf, b:+tr.dataset.asnew}; });
    ...
    [2000,2200,2400,2600,2800,3000,3200,3400].forEach(function(v){ s+='<line x1="'+X(v)+'" y1="34" x2="'+X(v)+'" y2="'+(60+rows.length*44)+'" stroke="#EEEFF1"/><text x="'+X(v)+'" y="'+(78+rows.length*44)+'" font-size="11" fill="#9EA2AA" text-anchor="middle">$'+v.toLocaleString()+'</text>'; });
    ...
    var t0=X(2800), yb=50+rows.length*44; s+='<line x1="'+t0+'" y1="34" x2="'+t0+'" y2="'+(yb-10)+'" stroke="#CF3F22" stroke-width="2"/><text x="'+t0+'" y="22" font-size="11.5" fill="#CF3F22" text-anchor="middle" font-weight="500">Thomson Reserve, $2,800 brand new</text></svg>';
