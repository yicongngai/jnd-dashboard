...
(function(root){
 ...
 function merge(tiles,bounds,verified={},houses={},landedHeight=6,hdb={},refresh={}){
  ...
  const features=[];for(const [key,g] of groups){let geometry=g.geometry;if(g.pieces.length>1){const merged=P.combine(g.pieces,[(bounds[0]+bounds[2])/2,(bounds[1]+bounds[3])/2]);geometry={type:'MultiPolygon',coordinates:merged};}const candidate=verified[g.properties.osmId],v=candidate&&(!candidate.matchId||candidate.matchId===key)?candidate:null,p={...g.properties};if(v){p.floors=v.floors;p.name=v.name;p.floorSource=v.source;}
   ...
   const houseType=houses[key]||(['house','detached','semidetached_house','terrace','bungalow'].includes(p.class)?p.class:null);if(houseType){p.houseType=houseType;if(!p.name)p.name='Landed home';}
   if(h.quality==='unknown'&&houseType&&landedHeight>0&&base===0&&!p.hasParts&&!p.parent){
    h.height=landedHeight;h.quality='assumed';h.source=`Source-classified ${houseType}. Height missing: illustrative ${landedHeight/3}-storey × 3 m = ${landedHeight} m landed-home estimate. Edit this height to match the actual home. Roof slope and attic are not measured.`;p.assumedLanded=true;
   }
