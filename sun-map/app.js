...
'use strict';
const $=id=>document.getElementById(id),home=[103.8994083,1.3005724],empty=()=>({type:'FeatureCollection',features:[]}),collection=features=>({type:'FeatureCollection',features});
...
const heightEdits=new Map(),buildingWorker=new Worker('building-worker.js?v=23'),shadowWorker=new Worker('shadow-worker.js?v=23');
fetch('data/future-projects.json?v=23').then(r=>r.json()).then(d=>{futureCatalog=d.projects;renderFutureProjects();}).catch(()=>{$('future-status').textContent='Future-project catalogue unavailable.';});
let places=[];fetch('data/places.json').then(r=>r.json()).then(x=>places=x).catch(()=>{});
...
function counts(){if(coverageCounts)return coverageCounts;const local=features.filter(f=>f.properties.quality!=='outline'&&study&&SolarGeometry.intersects(SolarGeometry.bbox(f.geometry),study));return coverageCounts={total:local.length,recorded:local.filter(f=>f.properties.quality==='tagged').length,estimated:local.filter(f=>f.properties.quality==='derived').length,landed:local.filter(f=>f.properties.quality==='assumed').length,missing:local.filter(f=>f.properties.quality==='unknown').length,future:local.filter(f=>f.properties.future).length,manual:local.filter(f=>f.properties.quality==='manual').length};}
function status(){const c=counts();$('status').textContent=loading?'Updating buildings for this view…':overview?'Pan across Singapore. Zoom in to see buildings and shadows.':loadError||`${c.total.toLocaleString()} footprints in view${c.future?' · '+c.future+' future building sections':''} · ${c.recorded} recorded heights · ${c.estimated} estimated${estimates?'':' (excluded)'}${c.landed?' · '+c.landed+' landed estimates':''}${c.manual?' · '+c.manual+' user-entered':''} · ${c.missing} missing.`;$('quality-note').textContent=notice||'Missing heights cast no shadow. Clear ground does not confirm sunlight.';document.body.dataset.loading=String(loading);}
...
let chartKey='';
function update(){if(!SolarEngine.validDate($('date').value)){stop();$('date').setCustomValidity('Choose a real date from 2000 to 2100.');$('date').reportValidity();++revision;clearShadows();notice='Choose a valid study date.';status();return;}$('date').setCustomValidity('');
 if((completed&&completed.date!==$('date').value)||(inFlight?.epoch===sceneEpoch&&inFlight.date!==$('date').value))clearShadows();if(!completed)renderReadout();scheduleShadows(sun());
}
function renderReadout(day=$('date').value,minute=Number($('time').value)){
 ...
 if(ready){map.setLight({anchor:'map',position:[1.5,s.bearing,Math.max(0,Math.min(90,90-s.altitude))],intensity:.22,color:'#ffffff'});const end=SolarGeometry.offset(center,Math.sin(s.bearing*Math.PI/180)*200,Math.cos(s.bearing*Math.PI/180)*200);map.getSource('sun-ray').setData(collection(s.altitude>0?[{type:'Feature',properties:{},geometry:{type:'LineString',coordinates:[center,end]}}]:[]));}
}
...
timeline.onpointerdown=e=>{if(e.button!==0||e.isPrimary===false)return;e.preventDefault();scrubPointer=e.pointerId;scrubStats.inputs=0;scrubStats.frames=0;timeline.focus({preventScroll:true});timeline.setPointerCapture(e.pointerId);scrubAt(e);};
...
function requestView(force=false,name){
 ...
 if(!force&&((loading&&ViewportPolicy.contains(requestedViewport,visible))||(!loading&&!loadError&&ViewportPolicy.contains(loadedViewport,visible))))return;
 ...
 notice='Updating this view · Previous study remains visible until ready.';status();buildingWorker.postMessage({id,center:loadArea.next.center,study:plan.study,bounds:plan.bounds,future:includeFuture,landedHeight:$('landed').checked?Number($('landed-height').value):0});
}
...
buildingWorker.onmessage=({data})=>{if(data.id!==loadId)return;loading=false;$('load-area').disabled=false;$('load-area').textContent='Refresh this view';
 ...
 for(const f of features){const edit=heightEdits.get(f.properties.key);if(edit)Object.assign(f.properties,edit);}
...
$('home').onclick=()=>{map.jumpTo({center:home,zoom:16.1});loadArea(home,'Amber Park');};
$('landed').onchange=$('landed-height').onchange=()=>requestView(true,$('place').textContent);
...
function showProject(index){const p=futureCatalog[index];if(!p)return;selected=null;$('building').hidden=false;$('building-name').textContent=p.name;
...
$('search').onsubmit=async e=>{e.preventDefault();const q=$('query').value.trim();if(q.length<2)return;const seq=++searchSeq,button=$('search').querySelector('button');button.disabled=true;$('results').textContent='Searching Singapore…';
 try{const futurePlaces=futureCatalog.filter(p=>p.center).map(p=>({name:p.name,lon:p.center[0],lat:p.center[1]}));let matches=[...futurePlaces,...places.filter(p=>!futurePlaces.some(f=>f.name.toLowerCase()===p.name.toLowerCase()))].filter(p=>p.name.toLowerCase().includes(q.toLowerCase())).sort((a,b)=>a.name.length-b.name.length).slice(0,8);if(!matches.length){matches=searchCache.get(q.toLowerCase());if(!matches){if(Date.now()-lastSearch<1100)await new Promise(r=>setTimeout(r,1100));lastSearch=Date.now();const response=await fetch('https://photon.komoot.io/api/?'+new URLSearchParams({q,lat:'1.35',lon:'103.82',bbox:ViewportPolicy.coverage.join(','),limit:'6'}),{signal:AbortSignal.timeout(10000)});if(!response.ok)throw Error('Search unavailable');const data=await response.json();matches=data.features.map(f=>({lon:f.geometry.coordinates[0],lat:f.geometry.coordinates[1],name:[f.properties.name,f.properties.housenumber,f.properties.street].filter(Boolean).join(' · ')})).filter(p=>p.lon>ViewportPolicy.coverage[0]&&p.lon<ViewportPolicy.coverage[2]&&p.lat>ViewportPolicy.coverage[1]&&p.lat<ViewportPolicy.coverage[3]);searchCache.set(q.toLowerCase(),matches);}}
 if(seq!==searchSeq)return;$('results').replaceChildren();if(!matches.length)$('results').textContent='No match. Pan the map; the study follows automatically.';for(const p of matches){const b=document.createElement('button');b.textContent=p.name;b.onclick=()=>choose(p);$('results').append(b);}
 }catch(e){$('results').textContent='Address search unavailable. Try a condo name, or pan the map.';}finally{if(seq===searchSeq)button.disabled=false;}
};
function selectBuilding(f){selected=features.find(x=>x.id===f.id);if(!selected)return;$('building').hidden=false;const p=selected.properties;$('building-name').textContent=p.name||'Building';$('height-source').textContent=p.source;$('height').value=p.height===null?'':p.height.toFixed(1);$('source-link').textContent='View building source ↗';$('source-link').hidden=!(p.floorSource||p.geometrySource);$('source-link').href=p.floorSource||p.geometrySource||'#';$('source-detail').textContent=(p.sources||[]).map(s=>s.dataset+(s.updated&&!s.updated.startsWith('2000-01-01')?' · source dated '+s.updated.slice(0,10):' · date unavailable')).filter((v,i,a)=>a.indexOf(v)===i).join('; ');$('height-form').hidden=p.quality==='outline';$('reset-height').hidden=p.quality==='outline';map.setFilter('selected-building',['==',['id'],selected.id]);}
$('close-building').onclick=()=>{$('building').hidden=true;selected=null;map.setFilter('selected-building',['==',['id'],-1]);};
function editHeight(height){if(!selected)throw Error('Select a building first');if(!Number.isFinite(height)||height<1||height>400||height<=selected.properties.baseHeight)throw Error('Roof height must be 1–400 m and higher than the building base.');const p={height,quality:'manual',source:`User-entered roof height: ${height} m above ground. Not independently verified.`};Object.assign(selected.properties,p);heightEdits.set(selected.properties.key,p);renderBuildings();selectBuilding(selected);update();}
...
try{
 ...
 map.on('load',()=>{for(const id of ['building-data','shadow-data','boundary-data','study-point','sun-ray','future-projects'])map.addSource(id,{type:'geojson',data:empty()});
  ...
  map.addLayer({id:'sun-ray-line',type:'line',source:'sun-ray',paint:{'line-color':'#CF3F22','line-width':3,'line-dasharray':[2,2]}});
  ...
  map.on('click',e=>{const projects=map.queryRenderedFeatures(e.point,{layers:['future-markers']});if(projects.length){showProject(projects[0].id);return;}const hits=map.queryRenderedFeatures(e.point,{layers:['buildings','unknown-footprints']});if(hits.length)selectBuilding(hits[0]);});map.on('mouseenter','unknown-footprints',()=>map.getCanvas().style.cursor='pointer');map.on('mouseleave','unknown-footprints',()=>map.getCanvas().style.cursor='');ready=true;renderFutureProjects();loadArea(home,'Amber Park');});
