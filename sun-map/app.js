...
const heightEdits=new Map(),buildingWorker=new Worker('building-worker.js?v=24'),shadowWorker=new Worker('shadow-worker.js?v=24');
fetch('data/future-projects.json?v=24').then(r=>r.json()).then(d=>{futureCatalog=d.projects;renderFutureProjects();}).catch(()=>{$('future-status').textContent='Future-project catalogue unavailable.';});
...
function showProject(index){const p=futureCatalog[index];if(!p)return;selected=null;$('building').hidden=false;$('building-name').textContent=p.name;$('building-address').textContent='Select an individual building for its block address.';$('address-source').hidden=true;$('height-edit-note').hidden=true;
